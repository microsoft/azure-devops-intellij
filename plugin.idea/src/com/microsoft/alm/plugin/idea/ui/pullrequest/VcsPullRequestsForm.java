...
public class VcsPullRequestsForm {
    ...
    public void setConnectionStatus(final boolean connected, final boolean authenticating, final boolean authenticated, final boolean loading) {
        updateStatusText(connected, authenticating, authenticated, loading);
    }

    private void updateStatusText(final boolean connected, final boolean authenticating, final boolean authenticated, final boolean loading) {
        if (!connected) {
    ...
    public void setLoadingErrors(final boolean loadingErrors) {
        if (loadingErrors) {
            ...
            statusLink.setText(TfPluginBundle.message(TfPluginBundle.KEY_VCS_PR_RETRY));
            statusLink.setVisible(true);
        }
    }
